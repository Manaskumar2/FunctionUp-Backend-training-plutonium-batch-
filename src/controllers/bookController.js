...
const Bookdetails=require('../models/newBookModel')
...
// }
 const createBook = async function(req,res){
    let Books = req.body
    let saveBook= await Bookdetails.create(Books)
    res.send({data:saveBook})

 }
 const booklist=  async function(req,res){
    let allbooks = await Bookdetails.find().select({bookName:1,authorName:1,_id :0})
    res.send({msg:allbooks})
...
const getParticularBooks= async function(req,res){
 ...
 res.send({msg:particularBook})
    ...
    // let allBooks= await Bookdetails.find({$or:[{prices:{indianPrice:{$eq:"100INR","200INR","500INR"}}})
    // res.send({msg:allBooks})
    //let allBook=await bookModel.find({prices:{indian:{$eq:"1600INR" }}})
